...
Make_Shape_Subclass( "Torus", Shape );
    ...
    {	 
      var circle_points = [];
      for( var i = 0; i <= rows; i++ )   circle_points.push( vec3( 4 + Math.cos( i/rows * 2*Math.PI )/8, 0, Math.sin( i/rows * 2*Math.PI )/8 ) );
      
      Patch.prototype.populate( recipient, rows, columns, 360, circle_points, false, points_transform );     
    }
...
Make_Shape_Subclass( "N_Polygon", Triangle_Fan );
    ...
    {	 
      ...
      for( var i = 0; i < n; i++ )     this.add_fan_point( recipient, offset, vec3( Math.cos( i/n * 2*Math.PI ), 0, Math.sin( i/n * 2*Math.PI ) ), vec3(0, 1, 0), vec2( .5 + .5 * Math.cos( i/n * 2*Math.PI ), .5 + .5 * Math.sin( i/n * 2*Math.PI ) ) ); 
